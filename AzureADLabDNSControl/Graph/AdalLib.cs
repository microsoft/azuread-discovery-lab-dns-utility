...
namespace Graph
{
    public static class AdalLib
    {
        ...
        {
            var credential = new ClientCredential(Startup.LabUserClientId, Startup.LabUserSecret);
            var authContext = new AuthenticationContext(string.Format(Startup.Authority, tenantId));
            var result = await authContext.AcquireTokenAsync(Startup.GraphResource, credential);
