var lTimezone, lWinTz, lTimezoneAbb, bSkipSBInit;
...
$(document).ajaxError(function (event, xhr, ajaxOptions, thrownError) {
    if (xhr.status == 401) {
        //re-authenticate
        SiteUtil.ShowMessage("Sorry, your authentication token has expired. Please refresh your browser.", "Authentication Expired", SiteUtil.AlertImages.warning);
        return;
    }
...
var SiteUtil = function () {
    ...
    function _utcToServerAndLocal(sDate, sInputFormatMask) {
        ...
        var serverDte = dte.format("MM/DD/YYYY h:mm A") + " UTC";
        var localDte = dte.local().format("MM/DD/YYYY h:mm A") + " " + lTimezoneAbb;
    ...
    function highlight(text, search) {
        ...
        if (search != null && search.length > 0) {
            var rgx = new RegExp('(' + search + ')', "ig");
            res = res.replace(rgx, "<span class='highlite'>$1</span>");
        }
